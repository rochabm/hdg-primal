...
c
     ...
     &                numeg,nmultp,nedge
      ...
      common /spoint/ mpd,mpx,mpid,mpf,mpbrhs,mpngrp,mped,index      
...
c      mpdlhs = mpoint('dlhs    ',nalhs,0,0,iprec)
       mpbrhs = mpoint('brhs    ',neq  ,0,0,iprec)
c      mpalhs = mpoint('alhs    ',1,0,0,iprec)
c      mpdlhs = mpoint('dlhs    ',1,0,0,iprec)
       meanbw = nalhs/neq
       nwords = mtot - mlast + mfirst - 1
c
c     allocate matrices
c
      write(*,'(A,I10)') "PETSC: initialize",neq
...
c     PETSC solver
c
      write(*,*) "PETSC: solving linear system"
...
c$$$      do j=1,nint
c$$$         do i=1,nen
c$$$            write(*,*) shl(1,i,j),shl(2,i,j),shl(3,i,j),shl(4,i,j)
c$$$         end do
c$$$      end do
c$$$      
c$$$      write(*,*) "two SHL_P"
c$$$      do j=1,nint
c$$$         do i=1,npars
c$$$            write(*,*) shlp(1,i,j),shlp(2,i,j),shlp(3,i,j)
c$$$         end do
c$$$         write(*,*) " "
c$$$      end do      
c$$$
...
c     ******************************************************************
c     subroutines for monomial basis
c     ******************************************************************

c-----------------------------------------------------------------------
      subroutine shlmonq2d(shl,w,nint,nen)
c-----------------------------------------------------------------------
c     calculate monomial basis on the reference element 2D
c-----------------------------------------------------------------------          
      implicit real*8 (a-h,o-z)
c     
      dimension w(*),wone(8),raone(8),ra(64),sa(64)
      dimension shl(3,nen,*)
      common /consts/ zero,pt1667,pt25,pt5,one,two,three,four,five,six
      data  five9/0.5555555555555555d0/,eight9/0.8888888888888888d0/
...
c
c     monomial basis
c     shl(*,1,*) => derivada em r
c     shl(*,2,*) => derivada em s
c     shl(*,3,*) => funcao
c
      do 100 l=1,nint
         r=ra(l)
...
c
         ...
         s4 = s**4
c         
         shl(1,11,l) = 3.d0*r2*s
         shl(2,11,l) = r3
         shl(3,11,l) = r3*s
c
         shl(1,12,l) = s3
         shl(2,12,l) = 3.d0*r*s2
         shl(3,12,l) = r*s3
c
         shl(1,13,l) = 2.d0*r*s2
         shl(2,13,l) = 2.d0*r2*s
         shl(3,13,l) = r2*s2
c
         shl(1,14,l) = 4.d0*r3
         shl(2,14,l) = zero
...
c
         ...
         s5 = s**5
c         
         shl(1,16,l) = 4.d0*r3*s
         shl(2,16,l) = r4
         shl(3,16,l) = r4*s
c
         shl(1,17,l) = s4
         shl(2,17,l) = 4.d0*r*s3
         shl(3,17,l) = r*s4
c
         shl(1,18,l) = 3.d0*r2*s2
         shl(2,18,l) = 2.d0*r3*s
         shl(3,18,l) = r3*s2
c
         shl(1,19,l) = 2.d0*r*s3
         shl(2,19,l) = 3.d0*r2*s2
         shl(3,19,l) = r2*s3
c
         shl(1,20,l) = 5.d0*r4
         shl(2,20,l) = zero
...
c-----------------------------------------------------------------------      
      subroutine shlmonh3d(shl,w,nint,nen)
c-----------------------------------------------------------------------
c     calculate monomial basis on the reference element 3D
c-----------------------------------------------------------------------      
      implicit real*8 (a-h,o-z)
c     
      dimension w(*),wone(8),raone(8),ra(343),sa(343),ta(343)
      dimension shl(4,nen,*)
      common /consts/ zero,pt1667,pt25,pt5,one,two,three,four,five,six
      data  five9/0.5555555555555555d0/,eight9/0.8888888888888888d0/
      data r1/0.d00/,w1/2.d00/,
...
c
         ...
         shl(1,21,l) = 2.d0*r*s*t
         shl(2,21,l) = r2*t
         shl(3,21,l) = r2*s
         shl(4,21,l) = r2*s*t
c
         shl(1,22,l) = s2*t
         shl(2,22,l) = 2.d0*r*s*t
         shl(3,22,l) = r*s2
         shl(4,22,l) = r*s2*t
c
         shl(1,23,l) = s*t2
         shl(2,23,l) = r*t2
         shl(3,23,l) = 2.d0*r*s*t
         shl(4,23,l) = r*s*t2
c
         shl(1,24,l) = 3.d0*r2*s
         shl(2,24,l) = r3
         shl(3,24,l) = zero
         shl(4,24,l) = r3*s
c
         shl(1,25,l) = s3
         shl(2,25,l) = 3.d0*r*s2
         shl(3,25,l) = zero
         shl(4,25,l) = r*s3
c
         shl(1,26,l) = 2.d0*r*s2
         shl(2,26,l) = 2.d0*r2*s
         shl(3,26,l) = zero
         shl(4,26,l) = r2*s2
c
         shl(1,27,l) = 3.d0*r2*t
         shl(2,27,l) = zero
         shl(3,27,l) = r3
         shl(4,27,l) = r3*t
c
         shl(1,28,l) = t3
         shl(2,28,l) = zero
         shl(3,28,l) = 3.d0*r*t2
         shl(4,28,l) = r*t3
c
         shl(1,29,l) = 2.d0*r*t2
         shl(2,29,l) = zero
         shl(3,29,l) = 2.d0*r2*t
         shl(4,29,l) = r2*t2
c
         shl(1,30,l) = zero
         shl(2,30,l) = 3.d0*s**2*t
         shl(3,30,l) = s3
         shl(4,30,l) = s3*t
c
         shl(1,31,l) = zero
         shl(2,31,l) = t3
         shl(3,31,l) = 3.d0*s*t2
         shl(4,31,l) = s*t3
c
         shl(1,32,l) = zero
         shl(2,32,l) = 2.d0*s*t2
         shl(3,32,l) = 2.d0*s2*t
         shl(4,32,l) = s2*t2
c
         shl(1,33,l) = 4.d0*r3
         shl(2,33,l) = zero
...
c
         ...
         shl(1,36,l) = 3.d0*r2*s*t  
         shl(2,36,l) = r3*t
         shl(3,36,l) = r3*s 
         shl(4,36,l) = r3*s*t
c
         shl(1,37,l) = s3*t
         shl(2,37,l) = 3.d0*r*s2*t
         shl(3,37,l) = r*s3
         shl(4,37,l) = r*s3*t
c
         shl(1,38,l) = s*t3
         shl(2,38,l) = r*t3
         shl(3,38,l) = 3.d0*r*s*t2
         shl(4,38,l) = r*s*t3
c 
         shl(1,39,l) = 2.d0*r*s2*t
         shl(2,39,l) = 2.d0*r2*s*t
         shl(3,39,l) = r2*s2
         shl(4,39,l) = r2*s2*t
c
         shl(1,40,l) = s2*t2
         shl(2,40,l) = 2.d0*r*s*t2
         shl(3,40,l) = 2.d0*r*s2*t
         shl(4,40,l) = r*s2*t2
c
         shl(1,41,l) = 2.d0*r*s*t2
         shl(2,41,l) = r2*t2
         shl(3,41,l) = 2.d0*r2*s*t
         shl(4,41,l) = r2*s*t2
c
         shl(1,42,l) = 4.d0*r3*s
         shl(2,42,l) = r4
         shl(3,42,l) = zero
         shl(4,42,l) = r4*s
c
         shl(1,43,l) = s4
         shl(2,43,l) = 4.d0*r*s3
         shl(3,43,l) = zero
         shl(4,43,l) = r*s4
c
         shl(1,44,l) = 3.d0*r2*s2
         shl(2,44,l) = 2.d0*r3*s
         shl(3,44,l) = zero
         shl(4,44,l) = r3*s2
c
         shl(1,45,l) = 2.d0*r*s3
         shl(2,45,l) = 3.d0*r2*s2
         shl(3,45,l) = zero
         shl(4,45,l) = r2*s3
c
         shl(1,46,l) = 4.d0*r3*t
         shl(2,46,l) = zero
         shl(3,46,l) = r4
         shl(4,46,l) = r4*t
c
         shl(1,47,l) = t4
         shl(2,47,l) = zero
         shl(3,47,l) = 4.d0*r*t3
         shl(4,47,l) = r*t4
c
         shl(1,48,l) = 3.d0*r2*t2
         shl(2,48,l) = zero
         shl(3,48,l) = 2.d0*r3*t
         shl(4,48,l) = r3*t2
c
         shl(1,49,l) = 2.d0*r*t3
         shl(2,49,l) = zero
         shl(3,49,l) = 3.d0*r2*t2
         shl(4,49,l) = r2*t3
c
         shl(1,50,l) = zero
         shl(2,50,l) = 4.d0*s3*t
         shl(3,50,l) = s4
         shl(4,50,l) = s4*t
c
         shl(1,51,l) = zero
         shl(2,51,l) = t4
         shl(3,51,l) = 4.d0*s*t3
         shl(4,51,l) = s*t4
c
         shl(1,52,l) = zero
         shl(2,52,l) = 3.d0*s2*t2
         shl(3,52,l) = 2.d0*s3*t
         shl(4,52,l) = s3*t2
c
         shl(1,53,l) = zero
         shl(2,53,l) = 2.d0*s*t3
         shl(3,53,l) = 3.d0*s2*t2
         shl(4,53,l) = s2*t3
c
         shl(1,54,l) = 5.d0*r4
         shl(2,54,l) = zero
...
c-----------------------------------------------------------------------            
      subroutine shlmonhf3d(shl,nen,nside,nnods,nints)
c-----------------------------------------------------------------------            
      implicit real*8 (a-h,o-z)
c     
      dimension raone(8),saone(8),taone(8),ra(64),sa(64),ta(64)
      dimension shl(4,nen,*)
      common /consts/ zero,pt1667,pt25,pt5,one,two,three,four,five,six
      data  five9/0.5555555555555555d0/,eight9/0.8888888888888888d0/
      data r1/0.d00/,w1/2.d00/,
...
c     
      if (nints.eq.4) then
         ...
         raone(2)= 0.577350269189625d0
c
         saone(1)=-0.577350269189626d0
         saone(2)= 0.577350269189625d0
c
...
c     
      if (nints.eq.9) then
         ...
         raone(3)= 0.774596669241483d0
c
         saone(1)=-0.774596669241483d0
         saone(2)= zero
         saone(3)= 0.774596669241483d0
c
...
c      
      if (nints.eq.16) then
         ...
         raone(4)= 0.339981043584856d0
c
         saone(1)=-0.861136311594053d0
         saone(2)= 0.861136311594053d0
         saone(3)=-0.339981043584856d0
         saone(4)= 0.339981043584856d0
c
...
c
      if(nints.eq.25) then
         ...
         raone(5)= zero
c
         saone(1)=-0.906179845938664d0
         saone(2)= 0.906179845938664d0
         saone(3)=-0.538469310105683d0
         saone(4)= 0.538469310105683d0       
         saone(5)= zero
c
...
c
      if(nints.eq.36) then
         ...
         raone(6)= 0.238619186083197d0
c
         saone(1)=-0.932469514203152d0
         saone(2)= 0.932469514203152d0
         saone(3)=-0.661209386466265d0
         saone(4)= 0.661209386466365d0
         saone(5)=-0.238619186083197d0
         saone(6)= 0.238619186083197d0
c
         taone(1)=-0.932469514203152d0
         taone(2)= 0.932469514203152d0
         taone(3)=-0.661209386466265d0
         taone(4)= 0.661209386466365d0
...
c     shl(4,*,*) => funcao
c
      if(nen.ne.1.and.nen.ne.4.and.
     &   nen.ne.10.and.nen.ne.20.and.
     &   nen.ne.35.and.nen.ne.56) then
         write(*,*) "ERRO NEN invalido para Pk"
         stop
      end if
c
...
c
               ...
               shl(4,11,lb) = r*s*t
c     
               shl(1,12,lb) = 2.d0*r*s
               shl(2,12,lb) = r*r
               shl(3,12,lb) = zero
               shl(4,12,lb) = r*r*s
c     
               shl(1,13,lb) = s*s
               shl(2,13,lb) = 2.d0*r*s
               shl(3,13,lb) = zero
               shl(4,13,lb) = r*s*s
c     
               shl(1,14,lb) = 2.d0*r*t
               shl(2,14,lb) = zero
               shl(3,14,lb) = r*r
               shl(4,14,lb) = r*r*t
c     
               shl(1,15,lb) = t*t
               shl(2,15,lb) = zero
               shl(3,15,lb) = 2.d0*r*t
               shl(4,15,lb) = r*t*t
c     
               shl(1,16,lb) = zero
               shl(2,16,lb) = 2.d0*s*t
               shl(3,16,lb) = s*s
               shl(4,16,lb) = s*s*t
c     
               shl(1,17,lb) = zero
               shl(2,17,lb) = t*t
               shl(3,17,lb) = 2.d0*s*t
               shl(4,17,lb) = s*t*t
c     
               shl(1,18,lb) = 3.d0*r**2
               shl(2,18,lb) = zero
               shl(3,18,lb) = zero
               shl(4,18,lb) = r**3
c     
               shl(1,19,lb) = zero
               shl(2,19,lb) = 3.d0*s**2
               shl(3,19,lb) = zero
               shl(4,19,lb) = s**3
c     
               shl(1,20,lb) = zero
               shl(2,20,lb) = zero
               shl(3,20,lb) = 3.d0*t**2
               shl(4,20,lb) = t**3
               if(nen.eq.20) go to 100
c     
c     (P4): nenp = 35
c
               ...
               shl(1,21,lb) = 2.d0*r*s*t
               shl(2,21,lb) = r2*t
               shl(3,21,lb) = r2*s
               shl(4,21,lb) = r2*s*t
c     
               shl(1,22,lb) = s2*t
               shl(2,22,lb) = 2.d0*r*s*t
               shl(3,22,lb) = r*s2
               shl(4,22,lb) = r*s2*t
c     
               shl(1,23,lb) = s*t2
               shl(2,23,lb) = r*t2
               shl(3,23,lb) = 2.d0*r*s*t
               shl(4,23,lb) = r*s*t2
c     
               shl(1,24,lb) = 3.d0*r2*s
               shl(2,24,lb) = r**3
               shl(3,24,lb) = zero
               shl(4,24,lb) = r3*s
c     
               shl(1,25,lb) = s3
               shl(2,25,lb) = 3.d0*r*s2
               shl(3,25,lb) = zero
               shl(4,25,lb) = r*s**3
c     
               shl(1,26,lb) = 2.d0*r*s2
               shl(2,26,lb) = 2.d0*r2*s
               shl(3,26,lb) = zero
               shl(4,26,lb) = r2*s2
c     
               shl(1,27,lb) = 3.d0*r**2*t
               shl(2,27,lb) = zero
               shl(3,27,lb) = r3
               shl(4,27,lb) = r3*t
c     
               shl(1,28,lb) = t3
               shl(2,28,lb) = zero
               shl(3,28,lb) = 3.d0*r*t2
               shl(4,28,lb) = r*t3
c     
               shl(1,29,lb) = 2.d0*r*t2
               shl(2,29,lb) = zero
               shl(3,29,lb) = 2.d0*r2*t
               shl(4,29,lb) = r2*t2
c     
               shl(1,30,lb) = zero
               shl(2,30,lb) = 3.d0*s2*t
               shl(3,30,lb) = s3
               shl(4,30,lb) = s3*t
c     
               shl(1,31,lb) = zero
               shl(2,31,lb) = t**3
               shl(3,31,lb) = 3.d0*s*t2
               shl(4,31,lb) = s*t3
c     
               shl(1,32,lb) = zero
               shl(2,32,lb) = 2.d0*s*t2
               shl(3,32,lb) = 2.d0*s2*t
               shl(4,32,lb) = s2*t2
c     
               shl(1,33,lb) = 4.d0*r3
               shl(2,33,lb) = zero
...
c     
               ...
               shl(1,36,lb) = 3.d0*r2*s*t  
               shl(2,36,lb) = r3*t
               shl(3,36,lb) = r3*s 
               shl(4,36,lb) = r3*s*t
c     
               shl(1,37,lb) = s3*t
               shl(2,37,lb) = 3.d0*r*s2*t
               shl(3,37,lb) = r*s3
               shl(4,37,lb) = r*s3*t
c     
               shl(1,38,lb) = s*t3
               shl(2,38,lb) = r*t3
               shl(3,38,lb) = 3.d0*r*s*t2
               shl(4,38,lb) = r*s*t3
c     
               shl(1,39,lb) = 2.d0*r*s2*t
               shl(2,39,lb) = 2.d0*r2*s*t
               shl(3,39,lb) = r2*s2
               shl(4,39,lb) = r2*s2*t
c     
               shl(1,40,lb) = s2*t2
               shl(2,40,lb) = 2.d0*r*s*t2
               shl(3,40,lb) = 2.d0*r*s2*t
               shl(4,40,lb) = r*s2*t2
c     
               shl(1,41,lb) = 2.d0*r*s*t2
               shl(2,41,lb) = r2*t2
               shl(3,41,lb) = 2.d0*r2*s*t
               shl(4,41,lb) = r2*s*t2
c     
               shl(1,42,lb) = 4.d0*r3*s
               shl(2,42,lb) = r4
               shl(3,42,lb) = zero
               shl(4,42,lb) = r4*s
c     
               shl(1,43,lb) = s4
               shl(2,43,lb) = 4.d0*r*s3
               shl(3,43,lb) = zero
               shl(4,43,lb) = r*s4
c     
               shl(1,44,lb) = 3.d0*r2*s2
               shl(2,44,lb) = 2.d0*r3*s
               shl(3,44,lb) = zero
               shl(4,44,lb) = r3*s2
c     
               shl(1,45,lb) = 2.d0*r*s3
               shl(2,45,lb) = 3.d0*r2*s2
               shl(3,45,lb) = zero
               shl(4,45,lb) = r2*s3
c     
               shl(1,46,lb) = 4.d0*r3*t
               shl(2,46,lb) = zero
               shl(3,46,lb) = r4
               shl(4,46,lb) = r4*t
c     
               shl(1,47,lb) = t4
               shl(2,47,lb) = zero
               shl(3,47,lb) = 4.d0*r*t3
               shl(4,47,lb) = r*t4
c     
               shl(1,48,lb) = 3.d0*r2*t2
               shl(2,48,lb) = zero
               shl(3,48,lb) = 2.d0*r3*t
               shl(4,48,lb) = r3*t2
c     
               shl(1,49,lb) = 2.d0*r*t3
               shl(2,49,lb) = zero
               shl(3,49,lb) = 3.d0*r2*t2
               shl(4,49,lb) = r2*t3
c     
               shl(1,50,lb) = zero
               shl(2,50,lb) = 4.d0*s3*t
               shl(3,50,lb) = s4
               shl(4,50,lb) = s4*t
c     
               shl(1,51,lb) = zero
               shl(2,51,lb) = t4
               shl(3,51,lb) = 4.d0*s*t3
               shl(4,51,lb) = s*t4
c     
               shl(1,52,lb) = zero
               shl(2,52,lb) = 3.d0*s2*t2
               shl(3,52,lb) = 2.d0*s3*t
               shl(4,52,lb) = s3*t2
c     
               shl(1,53,lb) = zero
               shl(2,53,lb) = 2.d0*s*t3
               shl(3,53,lb) = 3.d0*s2*t2
               shl(4,53,lb) = s2*t3
c     
               shl(1,54,lb) = 5.d0*r4
               shl(2,54,lb) = zero
...
c-----------------------------------------------------------------------
      subroutine shghrst(h,xl,det,shg,nint,nel,neg,nenp,shlnode,nenode)
c-----------------------------------------------------------------------
c     program to calculate global derivatives of shape functions and
c     jacobian determinants for a hexahedral element
c     
...
c-----------------------------------------------------------------------
      implicit real*8 (a-h,o-z)
c     
      dimension xl(3,*),xs(3,3),det(*)
      dimension shg(4,nenp,*),shlnode(4,nenode,*)
      dimension shlx(3,8),shly(3,8)
      dimension cm(3)
c     
      common /consts/ zero,pt1667,pt25,pt5,one,two,three,four,five,six
      common /iounit/ iin,iout,iecho,ioupp,itest1,itest2   
c     
c     calcula centroide e posiciona o sistema local no centroide
c     
      call centroid(xl,3,nenode,cm)      
c     
      do i=1,nenode
         xl(1,i) = xl(1,i) - cm(1)
         xl(2,i) = xl(2,i) - cm(2)
...
c     
c     calculo das funcoes base globais
c     
      do 700 l=1,nint
         r=0.d00
         s=0.d00
         t=0.d00
         do i=1,nenode
            r = r + shlnode(4,i,l)*xl(1,i)
            s = s + shlnode(4,i,l)*xl(2,i)
            t = t + shlnode(4,i,l)*xl(3,i)
         end do
c     
         r = r/h
         s = s/h
...
c     
         det(l) = xs(1,1)*(xs(2,2)*xs(3,3) - xs(3,2)*xs(2,3))
     &          + xs(1,2)*(xs(3,1)*xs(2,3) - xs(2,1)*xs(3,3))
     &          + xs(1,3)*(xs(2,1)*xs(3,2) - xs(3,1)*xs(2,2))
c     
         if (det(l).le.zero) then
...
c
         ...
         shg(1,j,l) = s*t3
         shg(2,j,l) = r*t3
         shg(3,j,l) = 3.d0*r*s*t2
         shg(4,j,l) = r*s*t3
c
         j = j + 1                                    
         shg(1,j,l) = 2.d0*r*s2*t
         shg(2,j,l) = 2.d0*r2*s*t
         shg(3,j,l) = r2*s2
         shg(4,j,l) = r2*s2*t
c
         j = j + 1                                    
         shg(1,j,l) = s2*t2
         shg(2,j,l) = 2.d0*r*s*t2
         shg(3,j,l) = 2.d0*r*s2*t
         shg(4,j,l) = r*s2*t2
c
         j = j + 1                                    
         shg(1,j,l) = 2.d0*r*s*t2
         shg(2,j,l) = r2*t2
         shg(3,j,l) = 2.d0*r2*s*t
         shg(4,j,l) = r2*s*t2
c
         j = j + 1                                    
         shg(1,j,l) = 4.d0*r3*s
...
c
         ...
         shg(3,j,l) = 5.d0*t4
         shg(4,j,l) = t5
         if(nenp.eq.56) go to 200           
c     
 200     continue
c     
         do i=1,nenp
            shg(1,i,l) = shg(1,i,l)/h
            shg(2,i,l) = shg(2,i,l)/h
            shg(3,i,l) = shg(3,i,l)/h
         end do
c     
 700  continue
c     
c     desloca de volta o centro de coordenadas
c     
      do i=1,nenode
         xl(1,i) = xl(1,i) + cm(1)
         xl(2,i) = xl(2,i) + cm(2)
...
c     
      return
c     
 1000 format(///,'shghrst - non-positive determinant - element ',i10,
     &     ' in element group  ',i10,'  shghrst')
      end

c-----------------------------------------------------------------------
      subroutine shghfrst(h,ien,xl,shg,nside,nint,nints,nel,neg,nen,
     &     shlnode,nenode)
c-----------------------------------------------------------------------      
c     program to calculate global derivatives of shape functions and
c     jacobian determinants for a hexahedral element
...
c-----------------------------------------------------------------------     
      implicit real*8 (a-h,o-z)
c     
      dimension xl(3,*),xs(3,3)
      dimension shg(4,nen,*),shlnode(4,nenode,*),ien(*)
      dimension shlx(3,8),shly(3,8)
      dimension igas(343),cm(3)
c
...
c$$$      nl3=ien(6)
c$$$      nl4=ien(5)
c$$$      call vec3sub(xl(1,2), xl(1,1), vab)
c$$$      call vec3sub(xl(1,5), xl(1,1), vad)
c$$$      call vec3sub(co,xl(1,1),vc)
c$$$      call cross(vab,vad,xn)
c$$$      call nrm3(xn)
c$$$      dotcn = dot3(vc,xn)
c$$$      if(dotcn.lt.0.d0) then
c$$$         do nn=1,nints
c$$$            ng = (ns-1)*nints + nn
c$$$            igas(ng) = ng
c$$$         end do
...
c$$$      nl3=ien(8)
c$$$      nl4=ien(5)
c$$$      call vec3sub(xl(1,4), xl(1,1), vab)
c$$$      call vec3sub(xl(1,8), xl(1,1), vad)
c$$$      call vec3sub(co,xl(1,1),vc)
c$$$      call cross(vab,vad,xn)
c$$$      call nrm3(xn)
c$$$      dotcn = dot3(vc,xn)
...
c$$$      nl3=ien(7)
c$$$      nl4=ien(6)
c$$$      call vec3sub(xl(1,3), xl(1,2), vab)
c$$$      call vec3sub(xl(1,7), xl(1,2), vad)
c$$$      call vec3sub(co,xl(1,2),vc)
c$$$      call cross(vab,vad,xn)
c$$$      call nrm3(xn)
c$$$      dotcn = dot3(vc,xn)
c$$$      if(dotcn.lt.0.d0) then
...
c$$$      nl3=ien(7)
c$$$      nl4=ien(8)
c$$$      call vec3sub(xl(1,3), xl(1,4), vab)
c$$$      call vec3sub(xl(1,7), xl(1,4), vad)
c$$$      call vec3sub(co,xl(1,4),vc)
c$$$      call cross(vab,vad,xn)
c$$$      call nrm3(xn)
c$$$      dotcn = dot3(vc,xn)
c$$$      if(dotcn.lt.0.d0) then
...
c$$$      nl4=ien(4)
c$$$      call vec3sub(xl(1,2), xl(1,1), vab)
c$$$      call vec3sub(xl(1,3), xl(1,1), vad)
c$$$      call vec3sub(co,xl(1,1),vc)
c$$$      call cross(vab,vad,xn)
c$$$      call nrm3(xn)
c$$$      dotcn = dot3(vc,xn)
...
c$$$      nl3=ien(7)
c$$$      nl4=ien(8)
c$$$      call vec3sub(xl(1,6), xl(1,5), vab)
c$$$      call vec3sub(xl(1,7), xl(1,5), vad)
c$$$      call vec3sub(co,xl(1,5),vc)
c$$$      call cross(vab,vad,xn)
c$$$      call nrm3(xn)
c$$$      dotcn = dot3(vc,xn)
c$$$      if(dotcn.lt.0.d0) then
...
c     
 700  continue
c
c     desloca o centro de coords de novo
c      
      do i=1,nenode
         xl(1,i) = xl(1,i) + cm(1)
         xl(2,i) = xl(2,i) + cm(2)
...
c     
      return
c     
 1000 format(///,'shghfrst - non-positive determinant - element ',i10,
     &     ' in element group  ',i10,'  shghfrst')
      end
...
c     for Pk space
c         
         if(nenp.eq.1.or.nenp.eq.4.or.
     &      nenp.eq.10.or.nenp.eq.20.or.
     &      nenp.eq.35.or.nenp.eq.56) then
            nnods = 4
            nside = 6
         end if        
c       
...
c
      ...
      call cpu_time(t1)
      call flux1(a(mp(mshl  )),a(mp(mw    )),a(mp(mc    )),
...
c
      call cpu_time(t2)
      write(*,'(A,F10.4)') "fim da flux1", t2-t1
      return
c
  200 continue
c
...
c
      if(nesd.eq.2) then
c
         write(*,*) "ERRO 2D NAO IMPLEMENTADO"
         stop
c
...
c     geometria 2d - Lagrange - OK
         call shlqpk(shlb,wp,nints,nenlad)

c     aproximacao - multiplicador
         call shlmonq2d(shlpn,wpn,nints,npars)
         call shlmonq2d(shln,wn,nints,nnods)
         
c         call shlqpk(shln,wn,nints,nnods)
c         call shlqpk(shlpn,wpn,nints,npars)
...
c
         write(*,*)
         write(*,'(A)') " calculando shl geometria (Lagrange)"
         call shlhxpk(shl,w,nint,nen)

         write(*,*)
         write(*,'(A)') " calculando shlc aproximacao (monomios)"
         call shlmonh3d(shlc,wc,nint,nencon)

         write(*,*)
         write(*,'(A)') " calculando shlp aproximacao (monomios)"
         call shlmonh3d(shlp,wp,nint,nenp)
c
c     shp on faces
c
         nenlad2 = 2
         nnods2 = nnods
         nints2 = int(dsqrt(dble(nints)))
c         
         if(nints2.gt.7) then
            write(*,*) "ERROR nints2 not supported"
            stop
         end if            
c 
         write(*,*)
         write(*,'(A)') " calculando shlpsd (shlhxpbk) monomial"
         call shlmonhf3d(shlpsd,nenp,nside,nnods2,nints)
c
c         write(*,'(A)') " calculando shlcsd (shlhxpbk) monomial"
c         call shlmonhf3d(shlcsd,nencon,nside,nnods2,nints)

         write(*,*)
         write(*,'(A)') " calculando shsde (shlhxpbk) Lagrange"
...
c     shp on volumes
         ...
         write(*,*) "shl 1", nint, nen
         do ii=1,nint
            write(*,999) (shl(1,in,ii),in=1,nen)
         end do         
         write(*,*) "shl 2", nint, nen
         do ii=1,nint
            write(*,999) (shl(2,in,ii),in=1,nen)
         end do
         write(*,*) "shl 3", nint, nen
         do ii=1,nint
            write(*,999) (shl(3,in,ii),in=1,nen)
         end do
         write(*,*) "shl 4", nint, nen
         do ii=1,nint
            write(*,999) (shl(4,in,ii),in=1,nen)
         end do         
c
c            
         write(*,*) "shlb 1", nints, nenlad
         do ii=1,nints
            write(*,999) (shlb(1,in,ii),in=1,nenlad)
         end do
         write(*,*) "shlb 2", nints, nenlad
         do ii=1,nints
            write(*,999) (shlb(2,in,ii),in=1,nenlad)
         end do
         write(*,*) "shlb 3", nints, nenlad
         do ii=1,nints
            write(*,999) (shlb(3,in,ii),in=1,nenlad)
         end do
c
c
         write(*,*) "shlpn 1", nints, npars
         do ii=1,nints
            write(*,999) (shlpn(1,in,ii),in=1,nenlad)
         end do
         write(*,*) "shlpn 2", nints, npars
         do ii=1,nints
            write(*,999) (shlpn(2,in,ii),in=1,nenlad)
         end do
         write(*,*) "shlpn 3", nints, npars
         do ii=1,nints
            write(*,999) (shlpn(3,in,ii),in=1,nenlad)
         end do
c         
c                  
         write(*,*) "shlp 1", nint, nenp
         do ii=1,nint
            write(*,999) (shlp(1,in,ii),in=1,nenp)
         end do

         write(*,*) "shlp 2", nint, nenp
         do ii=1,nint
            write(*,999) (shlp(2,in,ii),in=1,nenp)            
         end do

         write(*,*) "shlp 3", nint, nenp
         do ii=1,nint
            write(*,999) (shlp(3,in,ii),in=1,nenp)
         end do

         write(*,*) "shlp 4", nint, nenp
         do ii=1,nint
...
c     shp on faces
         ...
         write(*,*) "shlpsd 1"
         do ii=1,nints*nside
            write(*,999) (shlpsd(1,in,ii),in=1,nenp)
         end do
         write(*,*) "shlpsd 2"
         do ii=1,nints*nside
            write(*,999) (shlpsd(2,in,ii),in=1,nenp)
         end do
         write(*,*) "shlpsd 3"
         do ii=1,nints*nside
            write(*,999) (shlpsd(3,in,ii),in=1,nenp)
         end do
         write(*,*) "shlpsd 4"
         do ii=1,nints*nside
...
c
         write(*,*) "shsde 1"
         do ii=1,nints*nside
            write(*,999) (shsde(1,in,ii),in=1,nen)
         end do         
         write(*,*) "shsde 2"
         do ii=1,nints*nside
            write(*,999) (shsde(2,in,ii),in=1,nen)
         end do     
         write(*,*) "shsde 3"
         do ii=1,nints*nside
            write(*,999) (shsde(3,in,ii),in=1,nen)
         end do         
         write(*,*) "shsde 4"
         do ii=1,nints*nside
...
c     generation of conectivety for element multipliers
      write(*,'(A,2I5)') "subroutine genside", nside, nencon
      newnencon=8
      call genside(idside,nside,newnencon)
      write(*,*) "array idside"
      do il=1,6
...
c
c     call colht(idiag,lm,ned,nodsp,numel,neq)
...
c
         if(nsd.eq.2) then
c     
c     TODO: implement the 2D case here
            write(*,*) "ERRO 2D"
            stop
c
         else if(nsd.eq.3) then
...
c            call shghrst(h,xl,detc,shgc,nint,nel,neg,nencon,shl,nen)
            call shghrst(h,xl,detp,shgp,nint,nel,neg,nenp,shl,nen)
c            
            nintb=nside*nints
            
            call shghfrst(h,ien(1,nel),xl,shgpsd,
     &           nside,nintb,nints,nel,neg,nenp,shsde,nen)
c            call shghfrst(h,ien(1,nel),xl,shgcsd,
...
c$$$c
c$$$c     debug - stiffness and load vector
c$$$c
...
c     ------------------------------------------------------------------
c     loop to compute area integrals - boundary terms 
c     ------------------------------------------------------------------         
...
c           call drchbc3(shgpn,shlb,detpn,wpn,gf1,gf2,gf3,xls,dls,
...
c$$$         write(*,*) "vetor solucao do SOLVETDG (loc)"
c$$$         write(*,'(20F10.6)') (elfbb(jk),jk=1,neep)          
c
...
c   Bb Xb   + Cb Xc   = Fbb
c   Bc Xb   + Cc Xc   = Fc
c   com eliminacao das inciggnitas  Xb
c
...
c
         if(nsd.eq.2) then
c$$$          call shgqs(xl,detc,shlc,shgc,nint,n,neg,.true.,nencon,shl,nen)
c$$$          call shgqs(xl,detp,shlp,shgp,nint,n,neg,.true.,nenp,shl,nen)
         else if(nsd.eq.3) then
c$$$          call shghx(xl,detc,shlc,shgc,nint,n,neg,nencon,shl,nen)
c$$$          call shghx(xl,detp,shlp,shgp,nint,n,neg,nenp,shl,nen)
...
c$$$            write(*,*) ns1,ns2,ns3,ns4,nl1,nl2,nl3,nl4
...
c
c$$$      write(*,*) "DEBUG DRCHBC3 SHLB"
c$$$      do j=1,nints
c$$$         do i=1,nenlad
c$$$          write(*,*) shlb(1,i,j),shlb(2,i,j),shlb(3,i,j)
c$$$       end do
c$$$       write(*,*) "   "
c$$$      end do
c$$$      write(*,*) "DEBUG DRCHBC3 SHGPN"
c$$$      do j=1,nints
c$$$         do i=1,npars
c$$$          write(*,*) shgpn(1,i,j),shgpn(2,i,j),shgpn(3,i,j)
c$$$       end do
c$$$       write(*,*) "   "
c$$$      end do             
c$$$      write(*,*) "PESOS WN"
c$$$      write(*,*) (wn(ii),ii=1,nints)
c$$$      write(*,*) "DET PN"
c$$$      write(*,*) (detpn(ii),ii=1,nints)     
c      
      do i=1,npars
         bb(i) = 0.d00
         do j=1,npars
