...
c ----------------------------------------------------------------------
      ...
      character*4 ia
      parameter (ndim=800000000)
c
      common a(ndim)
      common /dictn/ ia(20000000)
c
      PetscErrorCode   ierr
...
c----------------------------------------------------------------------
      subroutine UserCreateKSPSolver(userctx)
c----------------------------------------------------------------------
c     Solve Ax=b
c----------------------------------------------------------------------
      use UserModule
...
c----------------------------------------------------------------------
      subroutine UserDoLinearSolver(userctx,sol,bflag)
c----------------------------------------------------------------------
c     Solve Ax=b
c----------------------------------------------------------------------
      use UserModule
...
c
      call KSPSetOperators(ksp,A,A,ierr)
c
c     Specify that will consider non-zero initial guess
c      
...
c           all local values at once-> MatSetValues
c-----------------------------------------------------------------------
      use UserModule
...
c
  100 continue
      ...
      write(*,*) "BCs", n,ne,nfd,ng1,ng2,(ib(i),i=1,m)
...
c
      ...
      common /iounit/ iin,ipp,ipmx,ieco,ilp,ilocal,interpl,ielmat,iwrite
c$$$c
c$$$      call move(shg,shl,4*nen*nint)
c$$$c    
c
...
c
c$$$      write(*,'(A)') "subroutine flnormp (depois da flux3primal)"
c$$$      call flnormp(a(mp(mien )),a(mpx       ),a(mp(mxl   )),
...
c
      do nel=1,numel
         do ns=1,6
            xnrml(1,ns,nel) = 0.d00
            xnrml(2,ns,nel) = 0.d00
...
c     (p^n , q) + dt*(f,q)
c
               elfbb(nbj1) = elfbb(nbj1) + djn*pss*dtime
c
c     element stiffness
c
               do i=1,nenp
...
c
            ...
            xn3 = xn(3)
c
c     armazena normal da face
c
            if(abs(xn1).le.5.0e-15) xn1 = 0.d00
            if(abs(xn2).le.5.0e-15) xn2 = 0.d00
            if(abs(xn3).le.5.0e-15) xn3 = 0.d00
            xnrml(1,ns,nel) = xn1
            xnrml(2,ns,nel) = xn2
            xnrml(3,ns,nel) = xn3            
c
...
c
                  nbj = ned*(j-1)
                  nbj1 = nbj+1
                  djx = shgpsd(1,j,lb)*detpn(ls)*wn(ls)*dtm1
...
c
     ...
     &                neep,nee)
c
c     armazena a inversa da matriz do elemento em aelm e dbel
c         
         do i=1,neep
            do j=1,neep
...
c         
         if(nel.eq.1) then
            write(ielmat,*) 'matriz do elemento',nel,nee
            do i=1,nee
               do j=1,nee
...
c ------------------------------------------------------------------------------
         call cpu_time(xode1)
!
!     TODO: TEM que mudar isso aqui
!     dar loop em ELEMENTS, NENP e guardar tudo num array de tam numnp,
!     depois dar loop em NODES (numnp) e resolver as EDOs         
!
         do i=1,numnp
            xstim(i) = 0.d00
         end do
...
c
            ...
            pnaz = 0.d00
            do j=1,nenp
               pna  = pna  + shgp(4,j,l)*faa(j)
               pnax = pnax + shgp(1,j,l)*faa(j)
               pnay = pnay + shgp(2,j,l)*faa(j)
               pnaz = pnaz + shgp(3,j,l)*faa(j)               
            end do
c
c     loop to compute volume integrals
c            
...
c
               ...
               nbj1 = nbj+1
c
c     (p^{n+1},q) + dt*a(p^{n+1},q) +  b(\lmbda^{n+1},q) = (p^n , q ) + dt*(f,q)               
c
               elfbb(nbj1) = elfbb(nbj1) + djn*pna
     &              - dtm0*(sigmal*pnax*djx +
     &                      sigmat*pnay*djy +
     &                      sigman*pnaz*djz)
     &              - dtm0*(gama*pna*djn)
            end do
...
c
c     compute by matrix-vector product
c     
c$$$      do i=1,nenp
c$$$         xsum = 0.d0
c$$$         do j=1,nenp
c$$$            xsum = xsum + akelm(i,j,nel) * faa(j)
c$$$         end do
c$$$         elfbb(i) = elfbb(i) + xsum
c$$$      end do         
         
c         write(*,*) "MASSA DO ELEM", nel, nenp, neep
c         do i=1,nenp
c            write(*,*) (akelm(i,j,nel),j=1,nenp) 
c         end do
c         write(*,*)
         
c      write(*,*) "elem",nel
c      write(*,*) (elfbb(i), i=1,nenp)
...
c     calcula sign para a aresta/face
c     
c$$$            call vec3sub(xl(1,ns2), xl(1,ns1), vab)
c$$$            call vec3sub(xl(1,ns4), xl(1,ns1), vad)
c$$$            call vec3sub(coo,xl(1,ns1),vc)
...
c
            ndgs = lado(ns,nel)
c
c$$$            if(ideg(1,ndgs).eq.1) then
c$$$               stop
c$$$               call drchbc3(shgpn,shlb,detpn,wn,
c$$$     &                      gf1,gf2,gf3,xls,dls,
c$$$     &                      gama,pi,nints,nenlad,npars)
c$$$c
c$$$               ngs = npars*(ndgs-1)
c$$$               nls = npars*(ns-1)
c$$$               do i=1,npars
...
c     
     ...
     &                + sigman*diz*xn3                  
c     
                  pna = pna  + shgpsd(4,i,lb)*faa(i)
                  pnax= pnax + shgpsd(1,i,lb)*faa(i)
                  pnay= pnay + shgpsd(2,i,lb)*faa(i)
                  pnaz= pnaz + shgpsd(3,i,lb)*faa(i)
               end do
c     
               gna = sigmal*pnax*xn1
     &             + sigmat*pnay*xn2
...
c     
c            if(nesd.eq.2) then
c               call oneshgp(xls,detpn,shlb,shlpn,shgpn,
c     &              nenlad,npars,nints,nesd,ns,nel,neg)
c            else if(nesd.eq.3) then
               call twoshgp(xls,detpn,shlb,shlpn,shgpn,
     &              nenlad,npars,nints,nesd,ns,nel,neg,xxn)
c            end if
c     
...
c
      write(filename,'(a,i5.5,a)') "saida_",iter,".vtk"      
      open(unit=66, file=filename)
c
      write(66,'(A)') "# vtk DataFile Version 2.0"
      write(66,'(A)') "HDG-Parabolic"
      write(66,'(A)') "ASCII"
...
c     escreve VTK
c      
      write(66,'(A,I8,A)') "POINTS",8*numel," float"
      do i=1,8*numel
         write(66,*) xnew(1,i), xnew(2,i), xnew(3,i)
      end do
      write(66,*)
      
      write(66,'(A,2I10)') "CELLS",numel,9*numel
      do nel=1,numel
         idn = 8*(nel-1)
         write(66,"(9I10)") 8, idn,   idn+1, idn+2, idn+3,
     &                         idn+4, idn+5, idn+6, idn+7
      end do
      write(66,*)

      write(66,'(A,I8)') "CELL_TYPES",numel
      do nel=1,numel
         write(66,"(I2)") 12
      end do
      write(66,*)

      write(66,'(A,I8)') "POINT_DATA",8*numel
...
c
            call uexafx3(xint,yint,zint,ue,duex,duey,duez,
     &                   eps,tempo,index)
c
...
c
c            write(*,*) n,l,u(1),ue(1),u(2),ue(2),pe,ue(3)
c           
            if(iwrite.ne.0) then
...
c
               dcat = dexp(-3.d00*pi2*tempo)
               dhse = co*(1.d00-dcat)*sx*sy*sz
c
c     calculo do erro do multiplicador
c               
...
c-----------------------------------------------------------------------
      subroutine drchbc3(shgpn,shlb,detpn,wn,ggf1,ggf2,ggf3,xls,dls,eps,
     &                   pi,nints,nenlad,npars)
c-----------------------------------------------------------------------
c     Dirichlet boundary conditions
...
c
         index = 1
         call uexafx3(x1,x2,x3,ue,duex,duey,duez,eps,tempo,index)
         dhse = ue(3)
c         
c        dhse = co*(1.d0-cat)*sx*sy*sz         
c        dhse = dexp(-3.d00*pi2*tempo)*sx*sy*sz         
c         
         do j=1,npars
...
c-----------------------------------------------------------------------
      subroutine uexafx3(x,y,z,ue,duex,duey,duez,eps,tempo,index)
c-----------------------------------------------------------------------
...
c
      go to (100) index
c
c     PROBLEM 3D: T = (co/3pi2)*(1-exp(-3 pi2 t))sin(pix)sin(piy)sin(piz)
c
 100  continue
      pi=4.d00*datan(1.d00)
      pi2=pi*pi
...
c
      ue(1) = 0.d0
      ue(2) = 0.d0
      ue(3) = co*(1.d0-cat)*sx*sy*sz
c      
      duex(1) = 0.d0
      duex(2) = 0.d0
      duex(3) = co*pi*(1.d00-cat)*cx*sy*sz
c
      duey(1) = 0.d0
      duey(2) = 0.d0
      duey(3) = co*pi*(1.d00-cat)*sx*cy*sz
c
      duez(1) = 0.d0
      duez(2) = 0.d0
      duez(3) = co*pi*(1.d00-cat)*sx*sy*cz
c
      go to 1001
c
 1001 continue
...
c      end do
      write(*,*) " faces por elemento (msh)", nside
      do nel=1,numel
         call local(ien(1,nel),x,xl,nen,nsd,nesd)
