...
c     all local values at once: MatSetValuesss     
c-----------------------------------------------------------------------
      use UserModule
...
c     TODO: implement this correctly, only assemble lm(j)>0  
c           VecSetValues(b,nee,indx,elresf,ADD_VALUES,ierr)      
c-----------------------------------------------------------------------
      use UserModule
      implicit real*8 (a-h,o-z)
c
...
ccc      nsv = 2
         nsv = 19        
         mp(mxvm) = mpoint('xvm     ',nenp ,numel  ,0     ,iprec)
         mp(mxsv) = mpoint('xsv     ',nsv  ,nenp   ,numel ,iprec)         
c
      endif
c
c     task calls
...
c     new monodomain
c
      dimension xvm(nenp,*), xsv(19,nenp,*), xdsv(19), xxsv(19)
c
      common /consts/ zero,pt1667,pt25,pt5,one,two,three,four,five,six
      common /iounit/ iin,ipp,ipmx,ieco,ilp,ilocal,interpl,ielmat,iwrite
      common /colhtc/ neq
...
c     
      do nel=1,numel
         do i=1,nenp
            call tt2006_init(nsv, xsv(1,i,nel))
ccc         call ms_init(nsv, xsv(1,i,nel))
            xvm(i,nel) = xsv(1,i,nel)
         end do        
      end do
c
c     initial condition
...
c$$$               end do
               ...
               call tt2006_equation(nsv, dt, xsv(1,i,nel), xdsv, xstim)
ccc               call ms_equation(nsv, dt, xsv(1,i,nel), xdsv, xstim)
c
c     advance with euler method
c
               do j=1,nsv
c
c     Euler method
...
c                  
                  if(j.ge.2.and.j.le.13) then
                     ...
                     xsv(j,i,nel) = xsv(j,i,nel) + dt * xdsv(j)
                  end if
               end do
c
            end do        
...
c
     ...
     &                     - dtm0*(pnax*djx + pnay*djy + pnaz*djz)
     &                     - dtm0*(gama*pna*djn)
            end do
         end do
...
c     
               gna=pnax*xn1 + pnay*xn2 + pnaz*xn3    
c     
...
c     solve system    
c     
      call UserDoLinearSolver(userctx,xbrhs)
c
